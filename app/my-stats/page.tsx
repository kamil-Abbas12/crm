...
export default function MyStatsPage() {
  ...
  const callsByDay = last7.map(d => ({
    ...
    calls: activities
      .filter(a => a.type === "call" && new Date(a.date).toDateString() === d)
          ...
          <div className="max-w-4xl mx-auto space-y-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                ...
                <div key={i} className="bg-[#0a0f1e] border border-[#1e3a5f]/40 rounded-xl p-4 text-center">
                  <p className="text-2xl mb-1">{s.icon}</p>
                  <p className="text-white text-2xl font-black tabular-nums">
                    {loading ? <span className="text-[#1e3a5f]">…</span> : s.value}
                  </p>
                  <p className="text-[#3a5a8a] text-xs mt-0.5">{s.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              {/* Calls bar chart */}
              <div className="bg-[#0a0f1e] border border-[#1e3a5f]/40 rounded-xl p-5">
                <h3 className="text-white font-black text-sm mb-4">Calls — Last 7 Days</h3>
                {loading ? (
                  <div className="h-32 bg-[#1e3a5f]/20 rounded animate-pulse" />
                ) : (
                  <div className="flex items-end gap-2 h-32">
                    {callsByDay.map((d, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <span className="text-[#3a5a8a] text-[10px] tabular-nums">{d.calls || ""}</span>
                        <div
                          className="w-full rounded-t transition-all duration-500"
                          style={{
                            height: `${(d.calls / maxCalls) * 96}px`,
                            minHeight: d.calls > 0 ? "4px" : "0",
                            backgroundColor: d.calls > 0 ? "#0066cc" : "#0d1829",
                          }}
                        />
                        <span className="text-[#3a5a8a] text-[10px]">{d.label}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="bg-[#0a0f1e] border border-[#1e3a5f]/40 rounded-xl p-5">
                ...
                {loading ? (
                  <div className="space-y-2">
                    {[...Array(4)].map((_, i) => (
                      <div key={i} className="h-6 bg-[#1e3a5f]/20 rounded animate-pulse" />
                    ))}
                  </div>
                  ...
                  <div className="space-y-3">
                    {stats.verticalBreakdown.map((v: any) => {
                      ...
                      const color = VERTICAL_COLORS[v._id] || "#4a7aaa";
                      return (
                        <div key={v._id}>
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: color }} />
                              <span className="text-[#8ab4d4] text-xs">{v._id || "Unknown"}</span>
                            </div>
                            <span className="text-white text-xs font-bold">{v.count} ({pct}%)</span>
                          </div>
                          <div className="h-1.5 bg-[#1e3a5f]/30 rounded-full">
                  ...
                  <p className="text-[#4a7aaa] text-sm text-center py-6">No leads yet.</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-[#0a0f1e] border border-[#1e3a5f]/40 rounded-xl p-5">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                {(["new","contacted","qualified","converted","lost"] as const).map(status => {
                  ...
                  return (
                    <div key={status} className="bg-[#1e3a5f]/10 border border-[#1e3a5f]/30 rounded-xl p-3 text-center">
                      <p className={`text-xl font-black tabular-nums ${colors[status]}`}>
                        {loading ? "…" : count}
                      </p>
                      <p className="text-[#3a5a8a] text-xs mt-0.5 capitalize">{status}</p>
                    </div>
                  );
                })}
              </div>
